...
import Note from "./analytics/Note";
...
const Viewnote = () => {
  ...
  const reads = async (_id) => {
    const res = await fetch(`/read/${_id}`, {
      method: "put",
      headers: {
        "Content-Type": "application/json",
      },
  ...
  const analytics = () => {
    navigator(`/analytics/${userIdentification}`);
  };
    ...
    <>
      <div className="container">
        <div className="row rows-cols-3">
          ...
          <div className="col">
            <button className="btn btn-outline-info" onClick={analytics}>
              Analytics
            </button>
          </div>
          <div className="col">
              ...
              <>
                <div className="col cols" key={elem._id}>
                  ...
                  <button
                    ...
                    onClick={() => download(elem._id)}
                  >
                    {downloads + " "}
                    <i className="fa-solid fa-down-long"></i>
                  </button>
                  <NavLink to={`/analytics/${identification}/${generateID}`}>
                    <button className="btn btn-outline-info">
